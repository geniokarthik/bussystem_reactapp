  ...
  const handlePress = (item) => {
    ...
    try {
      ...
      if (item.TYPE === 'direct') {
        ...
        `;
        parameters = [item.first_route_id, item.first_trip_number, item.first_route_id];
      } else {
        query = `
         WITH FirstLeg AS (
        ...
        `;
        parameters = [
          item.first_route_id, item.first_trip_number, item.first_route_id,item.first_route_id,
      ...
      db.transaction((tx) => {
        tx.executeSql(
          ...
          (_, result) => {
            ...
            console.log('Received Data in SearchList:', routes_detail); // Log only inside the callback
